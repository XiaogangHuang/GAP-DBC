...
// constructor
kdtree2::kdtree2(array2ddouble& data_in,bool rearrange_in,int dim_in)
  : the_data(data_in),
//    N  ( data_in.shape()[0] ),
//    dim( data_in.shape()[1] ),
...
kdtree2_node* kdtree2::build_tree_for_range(int l, int u, kdtree2_node* parent) {
  ...
  if ((u-l) <= bucketsize) {
	...
	// DIFFERENT TO THE ORIGINAL KDTREE2 CODE
	if(m <=l || m >= u)
	{
		m = (l + u) /2;
		//#ifdef _DEBUG
...
static const double INFS = 1.0e38;
...
void kdtree2_node::process_terminal_node(searchrecord& sr) {
  ...
  const array2ddouble& data = *sr.data;
